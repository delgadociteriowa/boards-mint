...
const GameCard: React.FC<GameCardProps> = ({background, title, paragraph, gameLink}) => (
  <div className={`${background} h-[300px] p-6 bg-stone-900 rounded-3xl bg-center bg-cover flex flex-col w-[250px]  grow`}>
    ...
    <Link className="lowercase block bg-stone-200/70 hover:bg-stone-200/90 py-4 rounded-full text-center no-underline text-stone-800 text-xl tracking-[3px] mt-auto" href={`/${gameLink}`}>new game</Link>
  </div>
);
