  ...
  return (
    <nav id="nav" className="nav w-[90%] my-0 mx-auto max-w-[1200px] py-5 px-0 grid grid-cols-[max-content_max-content] justify-between">
      ...
      <ul className="nav__links absolute bg-stone-200/90 inset-0 z-30 min-h-[500px] sm:min-h-[400px] py-0 pl-[5%] pr-0 grid gap-7 md:static md:min-h-0 md:p-0 md:bg-transparent md:grid-flow-col md:gap-[1em]">
        <NavigationItem href='/' color="text-stone-600 hover:text-stone-800" text="home"/>
        <NavigationItem href='/games' color="text-stone-600 hover:text-stone-800" text="games"/>
        <NavigationItem href='/about' color="text-stone-600 hover:text-stone-800" text="about"/>
        {!session ? 
          <NavigationItem href='/login' color="text-sky-700 hover:text-sky-500" text="login"/>
          :
          <NavigationItem href='/account' color="text-sky-700 hover:text-sky-500" text={session.user.username}/>
        }
      </ul>
