...
const OctoBoardSquare: React.FC<OctoBoardSquareProps> = ({cellContent, color, colorHover}) => {
  const squareBaseStyle = 'aspect-square min-w-6 min-h-6 flex items-center justify-center';
  return (
    <div className={`${cellContent.id} ${color} ${cellContent.piece && colorHover} ${squareBaseStyle} ${cellContent.piece && 'cursor-pointer'}`}>
      {
